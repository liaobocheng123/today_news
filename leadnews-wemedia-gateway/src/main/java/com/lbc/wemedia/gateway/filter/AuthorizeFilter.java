package com.lbc.wemedia.gateway.filter;

import com.lbc.wemedia.gateway.utils.AppJwtUtil;
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            if(result == 0 || result == -1){
                ...
                log.info("网关解析jwt令牌find userid:{} from uri:{} 。网关解析jwt令牌后将userId存入请求header中再进行路由转发",id,request.getURI());
                System.out.println("网关过滤器");
                //重新设置token中的userid到header中，便于后面的服务得到用户信息
